...
void CRaidAnalyzerDlg::OnBnClickedButtonJpg()
{
	...
	case enStart:
		...
		m_Data = new CAllocator(DEFAULT_SECTORS_SIZE*512,2000);
		...
		for (size_t iDisk = 0; iDisk < iCountHDD; ++iDisk)
		{
			...
			pReadDisk[iDisk].setBytesPerSector(512);
...
LRESULT CRaidAnalyzerDlg::OnShowCuttingDialog(WPARAM wParam, LPARAM lParam)
{
	...
	while (true)
	{
		...
		/*------------------******************------------------*/
		case enCuttingFiles:
			dwSize = setting->getFileCount() * SECTOR_SIZE;
			...
			for (UINT i = 1; i <= setting->getFileCount(); ++i)
			{
				...
				pFileCreater->setSize(i*SECTOR_SIZE);
			...
			if (iDlgResult == ID_YES)
			{
				...
				dwBlockSize = iFileNumber * SECTOR_SIZE;
		...
		//	system((char*)strTextOut.GetBuffer());
			...
			if (iDlgResult == ID_YES)
			{
				...
				dwSectorCount = dwBlockSize / SECTOR_SIZE;

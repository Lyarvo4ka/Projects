...
bool CopyDataToFile(IODevice * read_device, File * target_file, uint64_t offset, uint64_t copy_size)
{
	if (!read_device)
		return false;
	if (!target_file)
	...
	while (cur_pos < copy_size)
	{
		...
		read_device->setPosition(read_pos);
		bytesRead = read_device->ReadData(buffer, bytes_to_copy);
		if (bytesRead == 0)
			return false;
...
public:
	...
	void execute(const path_string & target_folder)
	{
		...
		while (true)
		{
			if (!(bResult = findHeaderOffset(offset, header_offset)))
				break;
	...
	uint64_t SaveToFile(const uint64_t header_offset, const path_string & target_name)
	{
		...
		while (true)
		{
			...
			qt_block.block_size = ntohl(qt_block.block_size);	// to_big_endian(qt_block.block_size);
			...
			CopyDataToFile(device_, &write_file, keyword_offset, write_size);
